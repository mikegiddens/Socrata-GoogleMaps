...
SocrataGoogleMaps.prototype.load = function (filter, cb) {
    ...
    $.getJSON(this.baseUrl + '/resource/' + this.table + '.json', filter, function (data) {
		var data1 = [];
		if (record && record.location && record.location.coordinates && record.location.coordinates[0] && record.location.coordinates[1])
		data = data1;
...
SocrataGoogleMaps.prototype.addMarker = function (record, idx) {
	...
	if(record && record.location && record.location.coordinates && record.location.coordinates[0] && record.location.coordinates[1]){
		...
		this._markers.push(new google.maps.Marker({
			position: new google.maps.LatLng(record.location.coordinates[1], record.location.coordinates[0]),
			map: this._map
		}));
	}
